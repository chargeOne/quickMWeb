...
//	Mobilebone.classAnimation = "pop";
//	Mobilebone.classAnimation = "fade";
    //Mobilebone.classAnimation = "flip";
    //Mobilebone.classAnimation = "turn";
    //Mobilebone.classAnimation = "flow";
	/**
	...
	Mobilebone.transition = function(pageInto, pageOut, back, options) {
		...
		if (pageInto != null && pageInto.classList) {				
			...
			if (title) {
				...
				/////pageInto.setAttribute("data-title", document.title);/*ryan去掉修改文件头*/
			}
			...
			["animationstart", "animationend"].forEach(function(animationkey, index) {
				...
				if (!store[pageid]) {
                    ...
                    if (typeof animition == "string" && params_in.root[animition]) {
                        pageInto.addEventListener(animateEventName, function(e) {
                            /*ryan 2015.3.12日修改,这里如不加判断,会监听所有webkit animate动画*/
                            if(e.animationName.indexOf("slide")!=-1)params_in.root[animition](this, this.classList.contains("in")? "into": "out");
                        });
                    } else if (typeof animition == "function") {
                        pageInto.addEventListener(animateEventName, function(e) {
                            /*ryan 2015.3.12日修改,这里如不加判断,会监听所有webkit animate动画*/
                            if(e.animationName.indexOf("slide")!=-1)animition(this, this.classList.contains("in")? "into": "out");
                        });
                    }
                }
			});
	...
	**/
	Mobilebone.createPage = function(dom_or_html, element_or_options, options) {
        ...
        ////is_root=true;/*ryan,根地址不缓存*/
        //console.log("data-reload!====>2",is_root);
		if (is_root == true) {
	...
	**/
	Mobilebone.init = function() {	
		...
		if (hash == "" || hash == "#") {
			...
			// as a ajax
			this.ajax({
				url: hash.replace("#", "").split("?")[0],//ryan修改避免有加参数?的情况
				dataType: "unknown",
				error: function() {
	...
	Mobilebone.handleTapEvent = function(event) {		
		...
		*/
		...
		if (/^#/.test(target.getAttribute("href")) == true) {
            ...
            ////attr_reload="root";/*ryan,根地址不缓存*/
            //console.log("data-reload!====>1",attr_reload);
			if ((attr_reload == null || attr_reload == "false") && store[clean_url]) {
                ...
                $("#test").val($("#test").val()+"-a1");
			} else {
				Mobilebone.ajax(target);
                $("#test").val($("#test").val()+"-a2");
			}
			event.preventDefault();	
		}
	};
	...
	**/
	window.addEventListener("popstate", function() {
        ...
        hash=hash.split("?")[0]//ryan+,修改避免有加参数?的情况
		if (hash == "") {
