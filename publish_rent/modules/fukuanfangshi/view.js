...
define(['backbone', 'text!publish_rent/modules/fukuanfangshi/view.html'], function (Backbone, nearbyViewTemplate) {
    var nearbyView = Backbone.View.extend({
        ...
        events : {//events相当坑,只保存了$(this.el)的dom结构，既<div>...</div>,外部不能用el.children()来填充主容器
            'click #fukuanfangshi .cyList li':'li_subMenu'//点击片区
        },
        render: function (e) {
            if(this.collection){
                if(e.result=="success"){
            ...
            this.trigger("fukuanfangshi:render", {result:e.result,data: e.data});
            this.stickitData();
            return this;
        },
        stickitData:function() {//将local storage中的缓存数据填入表格
            var $bt_li =  $("#fukuanfangshi").find("#Rthelist").find("li");
            var tempData = ryan.syswin.app.global.user;

            var pianqu=tempData.getTempData("fabuchuzu_xt","fukuanfangshi");
        ...
        li_subMenu:function(e){
            ...
            //将片区名称写入localStorage
            ryan.syswin.app.global.user.setTempData("fabuchuzu_xt","fukuanfangshi",cx_name+"&"+cx_id);
