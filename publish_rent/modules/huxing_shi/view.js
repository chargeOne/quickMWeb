define(['backbone', 'text!publish_rent/modules/huxing_shi/view.html'], function (Backbone, nearbyViewTemplate) {
    var nearbyView = Backbone.View.extend({
        ...
        events : {//events相当坑,只保存了$(this.el)的dom结构，既<div>...</div>,外部不能用el.children()来填充主容器
            ...
            'click #huxing_shi .cyList li':'li_subMenu'//选择几室
        },
        stickitData:function() {//将local storage中的缓存数据填入表格
            var $bt_li =  $("#huxing_shi").find("#Rthelist").find("li");
            var tempData = ryan.syswin.app.global.user;

            var pianqu=tempData.getTempData("fabuchuzu_bt","huxing_shi");
        ...
        render: function (e) {
            ...
            this.trigger("huxing_shi:render", {result:e.result,data:e.data});
            this.stickitData();
            return this;
        },
        li_subMenu:function(e){
            ...
            var huxing_shi_value = $childBtn.text().replace(/\s/gi,'');
            ...
            //将户型信息写入localStorage
            ryan.syswin.app.global.user.setTempData("fabuchuzu_bt","huxing_shi",huxing_shi_value);
            //setTimeout('ryan.syswin.app.loadPage("ting")',800);

            var that=this;
            clearTimeout(that.delayId);
            that.delayId=setTimeout(function(){
                ryan.syswin.app.loadPage("huxing_ting");
            },800);
