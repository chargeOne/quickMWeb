define(['backbone', 'text!publish_rent/modules/huxing_wei/view.html'], function (Backbone, nearbyViewTemplate) {
    var nearbyView = Backbone.View.extend({
        ...
        events : {//events相当坑,只保存了$(this.el)的dom结构，既<div>...</div>,外部不能用el.children()来填充主容器
            'click #huxing_wei .cyList li':'li_subMenu'
        },
        stickitData:function() {//将local storage中的缓存数据填入表格
            var $bt_li =  $("#huxing_wei").find("#Rthelist").find("li");
            var tempData = ryan.syswin.app.global.user;

            var pianqu=tempData.getTempData("fabuchuzu_bt","huxing_wei");
        ...
        render: function (e) {
            ...
            this.trigger("huxing_wei:render", {result:e.result,data:e.data});
            this.stickitData();
            return this;
        },
        li_subMenu:function(e){
            ...
            var huxing_wei_value = $childBtn.text().replace(/\s/gi,'');
            if(huxing_wei_value == "无"){
                huxing_wei_value = "0卫";
            }
            ...
            //将户型信息写入localStorage
            ryan.syswin.app.global.user.setTempData("fabuchuzu_bt","huxing_wei",huxing_wei_value);
