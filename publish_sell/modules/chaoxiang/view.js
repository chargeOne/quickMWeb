...
define(['backbone', 'text!publish_sell/modules/chaoxiang/view.html'], function (Backbone, nearbyViewTemplate) {
    var nearbyView = Backbone.View.extend({
        ...
        stickitData:function() {//将local storage中的缓存数据填入表格
            var $bt_li =  $("#chaoxiang").find("#Rthelist").find("li");
            var tempData = ryan.syswin.app.global.user;

            var pianqu=tempData.getTempData("fabuchushou_xt","chaoxiang");
            pianqu = ryan.syswin.app.inputIsNull(pianqu)?pianqu.split("&")[0]:"";
            $bt_li.each(function(i,v) {
                var li_text = $(v).text().replace(/\s/gi, '');
                if (pianqu&&pianqu!=""&&pianqu == li_text) {
        ...
        li_subMenu:function(e){
            ...
            //将片区名称写入localStorage
            ryan.syswin.app.global.user.setTempData("fabuchushou_xt","chaoxiang",cx_name+"&"+cx_id);
