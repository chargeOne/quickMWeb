define(['backbone', 'text!publish_sell/modules/fangyuanbiaoqian/view.html'], function (Backbone, nearbyViewTemplate) {
    var nearbyView = Backbone.View.extend({
        ...
        stickitData:function() {//将local storage中的缓存数据填入表格
            ...
            var baohanzujin=tempData.getTempData("fabuchushou_xt",this.$selfPage.attr("id"));
        ...
        next_step:function(e){
            ...
            temp_fybq_name = temp_fybq_name.substr(0,temp_fybq_name.length-1);
            temp_fybq_value = temp_fybq_value.substr(0,temp_fybq_value.length-1);
            if(temp_fybq_name && temp_fybq_value){
                this.selectCon = temp_fybq_name +"&"+temp_fybq_value;
            }

            //将户型信息写入localStorage
            ryan.syswin.app.global.user.setTempData("fabuchushou_xt","fangyuanbiaoqian",this.selectCon);
