define(['backbone', 'text!publish_sell/modules/wei/view.html'], function (Backbone, nearbyViewTemplate) {
    var nearbyView = Backbone.View.extend({
        ...
        events : {//events相当坑,只保存了$(this.el)的dom结构，既<div>...</div>,外部不能用el.children()来填充主容器
            'click #wei .cyList li':'li_subMenu'
        },
        stickitData:function() {//将local storage中的缓存数据填入表格
            var $bt_li =  $("#wei").find("#Rthelist").find("li");
            var tempData = ryan.syswin.app.global.user;

            var pianqu=tempData.getTempData("fabuchushou_bt","wei");
        ...
        render: function (e) {
            ...
            this.trigger("wei:render", {result:e.result,data:e.data});
            this.stickitData();
            return this;
        },
        li_subMenu:function(e){
            ...
            var wei_value = $childBtn.text().replace(/\s/gi,'');
            if(wei_value == "无"){
                wei_value = "0卫";
            }
            ...
            //将户型信息写入localStorage
            ryan.syswin.app.global.user.setTempData("fabuchushou_bt","wei",wei_value);
