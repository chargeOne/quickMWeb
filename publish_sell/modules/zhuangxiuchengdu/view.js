define(['backbone', 'text!publish_sell/modules/zhuangxiuchengdu/view.html'], function (Backbone, nearbyViewTemplate) {
    var nearbyView = Backbone.View.extend({
        ...
        events : {//events相当坑,只保存了$(this.el)的dom结构，既<div>...</div>,外部不能用el.children()来填充主容器
            'click #zhuangxiuchengdu .cyList li':'li_subMenu'
        },
        render: function (e) {
            if(this.collection){
                if(e.result=="success"){
            ...
            this.trigger("zhuangxiuchengdu:render", {result:e.result,data:e.data});
            this.stickitData();
            return this;
        },
        stickitData:function() {//将local storage中的缓存数据填入表格
            var $bt_li =  $("#zhuangxiuchengdu").find("#Rthelist").find("li");
            var tempData = ryan.syswin.app.global.user;

            var pianqu=tempData.getTempData("fabuchushou_xt","zhuangxiuchengdu");
        ...
        li_subMenu:function(e){
            ...
            var zxcd_name = $childBtn.text().replace(/\s/gi,'');
            var zxcd_id = $parentBtn.data("id");
            ...
            //将片区名称写入localStorage
            ryan.syswin.app.global.user.setTempData("fabuchushou_xt","zhuangxiuchengdu",zxcd_name+"&"+zxcd_id);
